...
/**
 * BASED ON
 * loopback-composer-connector/lib/filterparser.js
 */
...
class QueryBuilder {
     ...
     * filter0 = {'where':{'field1':'value1'}, limit: 10, skip: 0}
     ...
     * @public
     */
    static create(filter, resourceType) {
        let result = 'SELECT ' + resourceType;
        let filterKeys = Object.keys(filter);

        if (filterKeys.indexOf('where') >= 0) {
            // if there is an op
            result += ' WHERE ' + this.parseWhereCondition(filter.where);
        }

        if (filterKeys.indexOf('limit') >= 0) {
            result = this.parseWhereCondition(filter.where);
            // if there is an op
            result += ' LIMIT ' + filter.limit;
        }

        if (filterKeys.indexOf('skip') >= 0) {
            result = this.parseWhereCondition(filter.where);
            // if there is an op
    ...
    static parsePropertyValue(key, op, value) {
        ...
        if (typeof value === 'string') {
            ...
            result = '(' + key + ' ' + op + ' ' + (JSON.stringify(value)).replace(/"/g, '\'') + ')';
        } else {
    ...
    static parseObjectValue(key, keyObject) {
        ...
        if (queryOp === 'contains') {
            ...
            if (!(theValue instanceof Array) || theValue.length === 0) {
                throw new Error('The contains value is not an array or does not have at least one element');
            }
            const subConditions = theValue.map((value) => {
                if (!this.isPrimitiveTypeValue(value)) {
                    throw new Error('Unsupported data type for the between operator');
                }
                return this.parsePropertyValue(key, 'CONTAINS', [value]);
            });
            result = `(${subConditions.join(' OR ')})`;
        } else if (queryOp === 'between') {
